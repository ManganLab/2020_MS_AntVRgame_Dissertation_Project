...
public class OVRPlayerController : MonoBehaviour
{
    ...
    public virtual void UpdateMovement()
    {
        ...
        // Make footsteps play when moving
        GameObject mainCamera = GameObject.Find("CenterEyeAnchor");
        AudioSource footsteps = mainCamera.GetComponent<AudioSource>();

        //AudioSource[] audioSources = mainCamera.GetComponents<AudioSource>();
        //AudioSource footsteps = audioSources[0];
        ...
        if (EnableLinearMovement)
        {
            ...
            // If we are moving then do footsteps
            if (!(moveForward || moveBack || moveLeft || moveRight) &&
             (!(primaryAxis.y > 0.0f || primaryAxis.y < 0.0f || primaryAxis.x > 0.0f || primaryAxis.x > 0.0f)))
            {
                footsteps.Play();
            }
        }
